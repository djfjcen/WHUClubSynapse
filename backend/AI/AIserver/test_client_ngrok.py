...
def test_financial_bookkeeping():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return (parsed_entries is not None and len(parsed_entries) > 0 and
                    confirmation_message is not None and len(confirmation_message.strip()) > 0 and
                    original_input is not None and len(original_input.strip()) > 0)
        else:
            print(f"错误响应: {response.text}")
            return False
...
def test_generate_financial_report():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return (report_summary is not None and len(report_summary.strip()) > 0 and
